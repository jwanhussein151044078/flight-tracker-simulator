...
import blue from '../../Assets/airplaneblue.png';
import yellow from '../../Assets/airplaneyellow.png';
import * as turf from '@turf/turf';
//mapboxgl.accessToken = 'pk.eyJ1IjoianZhbmgiLCJhIjoiY2x0NmF1YmpwMDlnZDJpbWdmejN0aXh6byJ9.mJf8avBHcpMaK680ibpUQg';
let MAP = null;
export const initializeMap = (containerRef,options) => {
    if(!MAP){
        MAP = new mapboxgl.Map({
            accessToken : 'pk.eyJ1IjoianZhbmgiLCJhIjoiY2x0NmF1YmpwMDlnZDJpbWdmejN0aXh6byJ9.mJf8avBHcpMaK680ibpUQg',
        ...
        MAP.loadImage(blue,(error,image)=>{
            if(error){
                ...
                MAP.addImage('blue-airplane', image,{ sdf: true });
            }
        })
        ...
        map.addLayer({
            ...
            paint: {
                'icon-color': [
                    ...
                    '#4287f5', // Color for hover state
                    '#d9be25'     // Default color
                ]
               //'icon-color': '#a38907'
            }
        });
...
export const updateFlightss=(map,featureCollection,source_id='flights')=>{
    if(map && map.isSourceLoaded(source_id)){
        let startTime = null;
        const duration = 500;
        let oldFeatures = [...map.queryRenderedFeatures({layers: [source_id]})];
        const bearings = Array.from({ length: 6 }, () => 0);
